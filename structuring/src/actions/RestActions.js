...
import {
  ...
  MAGENTO_GET_STOREVIEWS,
  MAGENTO_GET_PRODUCT_REVIEW,
  MAGENTO_ADD_PRODUCT_REVIEW,
  MAGENTO_ADD_PRODUCT_TO_WISHLIST,
} from './types';
...
const getStores = async (dispatch) => {
  try {
    const storedata = await magento.guest.getStores();
    const displayStore = await getStoreToBeDisplayed(storedata);
    const currentStore = await setCurrentStoreView(storedata);
    dispatch({
      type: MAGENTO_GET_STOREVIEWS,
      payload:{storeViews:storedata,defaultStore:displayStore,currentStore:currentStore},
    });
  } catch (error) {
    logError(error);
  }
...
async function getStoreToBeDisplayed(storeData) {
  let storecode = await AsyncStorage.getItem('current_store_code');
  if (!storecode) {
    const dfltStore = storeData.filter((item)=>item.code=='default');
    storecode = dfltStore[0].name;
  }
  return storecode;
}

async function setCurrentStoreView(storeData) {
  let storecode = await AsyncStorage.getItem('current_store_code');
  let currentStore = [];
  if (!storecode) {
    const dfltStore = storeData.filter((item)=>item.code=='default');
    currentStore = dfltStore[0];
  } else{
    const dfltStore = storeData.filter((item)=>item.code==storecode);
    currentStore = dfltStore[0];
  }
  return currentStore;
}
...
export const getProductReview = ({ productId }) => (dispatch) => {
  dispatch({ type: MAGENTO_ADD_PRODUCT_REVIEW, payload: null });
  magento.guest
    .getProductReview(productId)
    .then((reviews) => {
      dispatch({ type: MAGENTO_GET_PRODUCT_REVIEW, payload:reviews });
    })
    .catch((error) => {
      logError(error);
    });
...
export const addProductReview =(review) => async (dispatch)=>{
  ...
  try{
    ...
    let reviewdata = Object.assign(review,revstate);
    reviewdata= {review:reviewdata};
    const result = await magento.admin.addProductReview(reviewdata);
    const reviewMessage = {status:"ok",data:"You submitted your review for moderation."};
    dispatch({ type: MAGENTO_ADD_PRODUCT_REVIEW, payload: reviewMessage });
  } catch(error){
    const reviewMessage = {status:"error",data:error.message};
    dispatch({ type: MAGENTO_ADD_PRODUCT_REVIEW, payload: reviewMessage });
  }
}

export const addProductToWishlist = (wishlist) => async (dispatch)=>{
  try{
    const result = await magento.admin.addProductToWishlist(wishlist);
    const message = result.name+' has been added to your Wish List';
    dispatch({ type: MAGENTO_ADD_PRODUCT_TO_WISHLIST, payload:{current:wishlist.productId,success:message} });
  } catch(error){
