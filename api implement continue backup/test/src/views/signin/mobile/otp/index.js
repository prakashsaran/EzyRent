...
const theme = useContext(ThemeContext);
  ...
  const resendSubmit = (code) =>{
    ...
    props.resendMobileOtp(mobile,'SI');
  }

  const submitOtp = () =>{
    const {mobile,mail} = props
    const userdata = {id:mobile.id,otp:inputOtp,device_token,device_type};
    ...
    <SafeAreaView style={styles.container(theme)}>
      <ScrollView>
        ...
        <View style={styles.preprops(theme)}>
          <Text style={styles.proplabel(theme)}>We have sent an OTP SMS to </Text>
        ...
        <TouchableOpacity style={styles.changeMobile(theme)} onPress={()=>{NavigationService.navigate(NAVIGATION_SIGN_IN_MOBILE_NUMBER_PATH)}}>
...
const mapStateToProps = ({ signin }) => {
  const { error, success, loading,mobile } = signin;
...
export default connect(mapStateToProps, { resendMobileOtp,signIn })(SignUpMobile);
